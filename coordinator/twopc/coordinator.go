...
// commitAll sends COMMIT to both shards concurrently with retries to prevent partial commit.
...
func (c *Coordinator) commitAll(txn models.Transaction, sourceShard, destShard shardmap.ShardInfo) (models.TransactionResult, error) {
	...
	// Commit both shards concurrently — critical for performance
	go func() {
		err := c.commitWithRetry(sourceShard.Address, txn.TxnID, constants.OpDebit, txn.Source, txn.Amount, sourceShard.ShardID)
		results <- commitResult{shardID: sourceShard.ShardID, err: err}
	}()
	go func() {
		err := c.commitWithRetry(destShard.Address, txn.TxnID, constants.OpCredit, txn.Destination, txn.Amount, destShard.ShardID)
		results <- commitResult{shardID: destShard.ShardID, err: err}
	}()

	// Wait for both commits to complete
	for i := 0; i < 2; i++ {
		r := <-results
		if r.err != nil {
			log.Printf("2pc: txn %s — WARNING: commit failed after retries on shard %s: %v", txn.TxnID, r.shardID, r.err)
		}
	}
